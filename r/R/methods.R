...
#
# NOTE: references the (currently unexported) rtemis SupervisedSession class via
# `rtemis:::`. Dependency wiring (Suggests/Imports + proper registration) is
# handled separately; rtemis stays CRAN-ready (class not exported).
...
#' @keywords internal
#' @noRd
plot_SupervisedSession <- function(
  x,
  title = NULL,
  theme = NULL,
  width = NULL,
...
# %% register plot.SupervisedSession ----

# Register the plot method on the (currently unexported) rtemis SupervisedSession
# class. getFromNamespace() avoids `:::` (no R CMD check NOTE) and keeps rtemis
# CRAN-ready, since the class need not be exported. This top-level assignment
# runs at namespace load, which registers the S3 method for `plot()` dispatch.
